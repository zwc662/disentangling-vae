...
TRAIN_LOSSES_LOGFILE = "train_var_losses.log"
...
class Trainer():
	...
	def __init__(self, model_var, model_vae, optimizer, loss_f,
		...
		self.device = device
		self.model_var = model_var.to(self.device)
		self.model_vae = model_vae.to(self.device)
		self.loss_f = loss_f
	...
	def __call__(self, data_loader,
		...
		for epoch in range(epochs):
			...
			if epoch % checkpoint_every == 0:
				save_model(self.model_var, self.save_dir,
	...
	def _train_iteration(self, data, storer):
		...
		try:
			recon_batch, latent_dist, latent_sample = self.model_vae(data)
			var_batch = self.model_var(data)
			loss = self.loss_f(data, recon_batch, var_batch, self.model_var.training,
